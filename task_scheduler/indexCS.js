...
const { runCuckooSearch } = require('./cs_algorithm/cs');
...
app.post('/schedule', async (req, res) => {
  if (cuckooMapping.length === 0) {
    cuckooMapping = runCuckooSearch(tasks.length, workers.length, tasks);
    console.log('📌 Cuckoo Search Mapping:', cuckooMapping);

    if (!Array.isArray(cuckooMapping) || cuckooMapping.length !== tasks.length) {
  ...
  const targetIndex = cuckooMapping[currentIndex];
  ...
  try {
    ...
    if (completedTasks === totalTasks) {
      ...
      console.log(`✅ All tasks completed with Cuckoo Search.`);
...
app.listen(8080, () => {
  console.log('🚀 Broker running on port 8080 (CUCKOO SEARCH ENABLED)');
});
